...
def plot_tree(decision_tree, parent_pt, node_txt):
    d_node = dict(boxstyle="sawtooth", fc="0.8")    # 决策节点的属性
    leaf_node = dict(boxstyle="round4", fc='0.8')    #决策树叶子节点的属性
...
if __name__ == '__main__':
    myData = pd.read_csv('iris.csv', encoding='gbk')
    column_count = dict([(ds, list(pd.unique(myData[ds]))) for ds in myData.iloc[:, :-1].columns])
    d_tree = create_tree(myData, column_count)
    create_plot(d_tree)
